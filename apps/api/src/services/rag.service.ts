...
export class RAGService {
  ...
  async checkUsageLimit(clerkId: string, email: string): Promise<{ allowed: boolean; count: number; limit: number }> {
   ...
   */
  async getUserConversations(clerkId: string, limit: number = 20): Promise<any[]> {
    const { data: user } = await this.supabase
      .from('users')
      .select('id')
      .eq('clerk_id', clerkId)
  ...
  private async reformulateQuery(
    ...
    const hasTechnicalKeywords = /next\.?js|react|typescript|middleware|component|api|server|client|route|proxy|edge|runtime/i.test(query);
...
Reformulated query:`;
      ...
      // Log for debugging
      console.log(`[Query Reformulation] Original: "${query}" → Reformulated: "${reformulated}"`);
   ...
   */
  private getResponseModePersona(mode: string): { persona: string; style: string } {
    const modes: Record<string, { persona: string; style: string }> = {
      formal: {
        persona: 'You are a technical documentation writer creating official reference material.',
...
- Balance friendliness with technical accuracy`
      ...
      'bimbingan-belajar': {
        persona: 'You are a patient coding instructor teaching concepts step-by-step.',
        style: `**Response Format:**
...
- Maximum 5-6 sentences total`
      ...
      'technical-deep-dive': {
        persona: 'You are a senior architect explaining implementation details.',
        style: `**Response Format:**
...
- Think: what fits on a sticky note?`
  ...
  async searchDocumentation(
    ...
    if (error) {
      ...
      console.warn('search_docs_with_context not available, falling back to search_docs');
      const { data: fallbackData, error: fallbackError } = await this.supabase.rpc('search_docs', {
        query_embedding: queryEmbedding,
   ...
   */
  private filterQualityChunks(chunks: SearchResult[], limit: number): SearchResult[] {
   ...
   */
  private async getExpandedContext(chunks: SearchResult[]): Promise<SearchResult[]> {
    ...
    let query = this.supabase
      .from('doc_chunks')
      .select('id, content, url, title, source, chunk_index, full_content, metadata');

    // Add OR conditions for each chunk's surrounding range
    for (const req of chunkRequests) {
      query = query.or(
        `and(url.eq.${req.url},chunk_index.gte.${Math.max(0, req.chunkIndex - 1)},chunk_index.lte.${req.chunkIndex + 1})`
      );
    }

    const { data: allSurroundingChunks } = await query.order('url').order('chunk_index', { ascending: true });
    ...
    for (const req of chunkRequests) {
      ...
      const relevantChunks = surroundingChunks.filter(sc => 
      ...
      if (relevantChunks.length > 0) {
        ...
        if (!seenUrls.has(urlKey)) {
          expandedChunks.push({
            ...
            full_content: relevantChunks[0]?.full_content || req.originalChunk.full_content,
          });
          seenUrls.add(urlKey);
        }
      } else {
  ...
  async generateAnswer(
    ...
    const searchResults = await this.searchDocumentation(searchQuery, source, 5);
...
- Be accurate - only use info from docs below${conversationHistory && conversationHistory.length > 0 ? ' and conversation history' : ''}
${conversationHistory && conversationHistory.length > 0 ? '- Remember our conversation - reference it if relevant\n' : ''}
${style}
