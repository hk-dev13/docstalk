...
import { MessageSquare, Plus, Search, ChevronRight, PanelLeftClose, PanelLeftOpen, History } from 'lucide-react';
    ...
    <>
      ...
      <div className={cn("p-4 flex items-center", isCollapsed ? "justify-center" : "justify-between")}>
        ...
        <Button
          ...
          className={cn("text-muted-foreground hover:text-foreground", isCollapsed && "h-10 w-10")}
        >
      ...
      <div className="flex-1 overflow-y-auto px-2 py-2 space-y-1 scrollbar-thin">
        {filteredConversations.length === 0 ? (
          <div className={cn("text-center text-muted-foreground", isCollapsed ? "pt-4" : "p-8")}>
            {!isCollapsed ? (
              <>
          ...
          filteredConversations.map((conv) => (
            ...
            >
              ...
              {isCollapsed ? (
                <MessageSquare className={cn("h-5 w-5 shrink-0", currentConversationId === conv.id ? "text-primary" : "text-muted-foreground")} />
              ) : (
                <div className="flex items-start gap-3 pl-2 w-full overflow-hidden">
                  <MessageSquare className={cn("h-4 w-4 mt-1 shrink-0", currentConversationId === conv.id ? "text-primary" : "text-muted-foreground")} />
                  <div className="flex-1 min-w-0">
                    <p className={cn("text-sm font-medium truncate", currentConversationId === conv.id ? "text-primary" : "text-foreground")}>
                      {conv.title}
                    </p>
